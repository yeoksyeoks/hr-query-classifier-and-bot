# ChromaDB compatibility fix - must be at the very top
import os
import sys

# Fix for ChromaDB on Streamlit Cloud
def fix_chromadb_compatibility():
    """Fix ChromaDB SQLite compatibility issues on Streamlit Cloud"""
    try:
        ...
        # If pysqlite3 is not available, install it
        try:
            import subprocess
            subprocess.check_call([sys.executable, "-m", "pip", "install", "pysqlite3-binary"])
            import pysqlite3
            sys.modules['sqlite3'] = sys.modules.pop('pysqlite3')
            print("Successfully installed and configured pysqlite3")
        except Exception as e:
            print(f"Could not fix ChromaDB compatibility: {e}")
            # Continue anyway - might work on some systems

# Apply the fix immediately
fix_chromadb_compatibility()
...
try:
    ...
    st.error(f"Classifier Import Error: {str(e)}")
...
try:
    ...
    st.error(f"RAG Bot Import Error: {str(e)}")
    RAG_AVAILABLE = False
...
def check_password():
    ...
    def password_entered():
        ...
        if st.session_state["password"] == "hrqueryclassifierandbot_studentid=2794647y":
...
def sidebar_navigation():
    with st.sidebar:
        ...
        if CLASSIFIER_AVAILABLE:
            ...
            st.error("❌ Classifier: Error")
            
        if RAG_AVAILABLE:
            st.success("✅ RAG Bot: Ready")
        else:
            st.error("❌ RAG Bot: Error")
...
def show_home():
    ...
    with col2:
        ...
        with status_placeholder.container():
            ...
            # Module Status
            st.metric("Classifier", "Ready" if CLASSIFIER_AVAILABLE else "Error")
            st.metric("RAG Bot", "Ready" if RAG_AVAILABLE else "Error")
...
def process_queries_with_live_updates(df_process, query_column, total_queries, progress_container, status_container):
    ...
    if not CLASSIFIER_AVAILABLE:
        raise RuntimeError("Classifier module not available")
...
def show_batch_classifier():
    ...
    if not CLASSIFIER_AVAILABLE:
        st.error("**Classifier Module Unavailable**: There was an error loading the classification module. Please check the logs for details.")
        return
    ...
    if uploaded_file is not None:
        try:
            ...
            if st.session_state['processing_state'] == 'ready':
                ...
                with col_go:
                    if st.button("Go to RAG Chatbot", type="primary", use_container_width=True, disabled=not RAG_AVAILABLE):
                        if RAG_AVAILABLE:
                            st.session_state['current_page'] = "RAG Chatbot"
                            st.rerun()
                        else:
...
def show_rag_chatbot():
    ...
    if not RAG_AVAILABLE:
        st.error("**RAG Bot Module Unavailable**: There was an error loading the RAG bot module. Please check the logs for details.")
        st.info("Common issues include missing dependencies or ChromaDB compatibility problems.")
        return
    ...
    if 'rag_bot' not in st.session_state:
        if st.button("Initialise RAG System", type="primary"):
            try:
                ...
                st.info("This might be due to ChromaDB compatibility issues. Check the system logs for more details.")
        return
    ...
    try:
        ...
        with col2:
            st.image("process_flow_mermaid_chart.png", 
                    caption="HR Query Analysis System - Process Flow")
