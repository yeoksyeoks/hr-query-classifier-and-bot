...
# Method 1: Try to fix ChromaDB compatibility without subprocess
def setup_chromadb_compatibility():
    ...
    try:
        ...
        if 'STREAMLIT' in os.environ or 'streamlit' in sys.modules:
            # Try to import pysqlite3 (should be installed via requirements.txt)
            import pysqlite3
    ...
    try:
        import sqlite3
        # Check if we can use the existing sqlite3
        return True
...
# Import functions with comprehensive error handling
@st.cache_resource
def import_modules():
    """Import modules with detailed error handling - cached to avoid repeated imports"""
    global CLASSIFIER_AVAILABLE, RAG_AVAILABLE
    ...
    try:
        from hr_classifier_py import (
            summarise_hr_query, 
            classify_hr_topic, 
            classify_system_process_policy, 
            process_hr_queries, 
    ...
    # Try to import RAG bot
    if chromadb_compatible:
        try:
            from hr_rag_bot_py import HRQueryRAGBot
            rag_available = True
            print("✅ RAG Bot module loaded successfully")
        except Exception as e:
            rag_available = False
            ...
            if "chromadb" in str(e).lower():
                print("💡 ChromaDB compatibility issue detected")
    else:
        rag_available = False
        print("❌ RAG Bot unavailable due to ChromaDB compatibility issues")
    
    return classifier_available, rag_available

# Import modules and set global flags
try:
    CLASSIFIER_AVAILABLE, RAG_AVAILABLE = import_modules()
except Exception as e:
    st.error(f"Critical error during module import: {e}")
    CLASSIFIER_AVAILABLE, RAG_AVAILABLE = False, False
...
def show_system_diagnostics():
    ...
    with st.sidebar:
        ...
        if chromadb_compatible:
            st.success("✅ ChromaDB: Compatible")
        else:
            st.error("❌ ChromaDB: Issues detected")
        ...
        with st.expander("Environment Info"):
            ...
            if 'STREAMLIT' in os.environ:
                st.text("Environment: Streamlit Cloud")
            else:
                st.text("Environment: Local/Other")
